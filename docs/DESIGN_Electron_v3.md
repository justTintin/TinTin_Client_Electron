...
- 地址栏高度 36px，圆角 8px，背景 `--surface-container`，focus 时 primary ring。
- 前进/后退/刷新为 icon 按钮。
- 解析按钮仅在抖音/B站/YouTube 页面显示，使用 accent 色。
- 下载图标显示当前下载数量 badge。
...
- 历史记录
- 下载管理
- Cookie Profile 切换（V3.1 规划）

侧边栏宽度 220px，背景 `--surface-container`。

### 5.3 主浏览区

- 使用 BrowserView / webview，独立 session partition `persist:tintin-browser`。
