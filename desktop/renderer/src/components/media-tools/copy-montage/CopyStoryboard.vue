...
// ═════════════════════════════════════════════════════════════
// CopyStoryboard.vue — 文案混剪分镜脚本公共显示组件（四步共用，状态不同）
...
//   material（第三步）= 只读 + 智能匹配绑定的素材镜头列表（copyMontageAssignLogic 方案C）
...
const props = defineProps<{ mode: 'edit' | 'voice' | 'material' | 'fx' }>()
...
// ── 分镜 tab 管理（多分镜：每 tab 一个脚本对应一个视频；上限 10；双击重命名；删除二次确认）──
  ...
  <div class="sb-block">
    ...
    <div class="sb-tabs">
      <div v-for="sb in storyboards" :key="sb.id" class="sb-tab" :class="{ active: sb.id === activeStoryboardId }"
        ...
        <button class="sb-tab-close" :class="{ warn: removingId === sb.id }" :title="removingId === sb.id ? '再次点击确认删除' : '删除该分镜'"
          @click.stop="onTabClose(sb.id)">{{ removingId === sb.id ? '确认' : '×' }}</button>
      </div>
    ...
    <div class="row between">
      <span class="sb-info">分镜脚本（AI 根据旁白梳理）{{ copyShots.length ? `：共 ${copyShots.length} 镜 ｜ 预计总时长 ${shotsTotalSec} 秒` : '' }}
            ...
            <div class="script-list-pane">
              ...
              <div class="script-list">
                <button v-for="o in scriptPickDlg.options" :key="o.id" class="script-row"
                  ...
                  <span class="script-meta">{{ o.shotCount }} 镜 · {{ o.savedAt }}</span>
                </button>
