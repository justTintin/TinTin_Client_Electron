...
// ── 字幕重切段后处理（2026-09-18 用户裁决：声音克隆完成后即处理；本地+LLM 路线）──

test('splitLongSubtitleLines：超长按逗号停顿重切；单停顿仍超硬切；短行原样', () => {
  assert.deepEqual(R.splitLongSubtitleLines(['短句一行。'], 20), ['短句一行。'])
  const long = '50毫米大驱动单元搭配DTS 2.0环绕声，沉浸式音效精准听声辨位。'
  assert.deepEqual(R.splitLongSubtitleLines([long], 20),
    ['50毫米大驱动单元搭配DTS 2.0环绕声，', '沉浸式音效精准听声辨位。'])
  const hard = R.splitLongSubtitleLines(['字'.repeat(50)], 20)
  assert.deepEqual(hard, ['字'.repeat(20), '字'.repeat(20), '字'.repeat(10)])
})

test('mergeTinySubtitleLines：短于下限并入前行；首行过短保留不丢字', () => {
  assert.deepEqual(R.mergeTinySubtitleLines(['前行内容足够长', '太短'], 8), ['前行内容足够长太短'])
  assert.deepEqual(R.mergeTinySubtitleLines(['太短', '第二行内容足够长'], 8), ['太短', '第二行内容足够长'])
})

test('planSubtitleLines：LLM 有效采用；漏字/加字回退本地规则拆句', () => {
  const text = '粉色限定登场。低延迟稳定传输。'
  const llm = ['粉色限定登场。', '低延迟稳定传输。']
  assert.deepEqual(R.planSubtitleLines(text, llm), llm)
  // 本地拆句会把句读替换为切分点（splitTextIntoSentences 口径）→ 回退产物按去标点等价校验
  const strip = (s) => s.replace(/[。！？.!?\n]/g, '')
  // 漏字 → 回退本地（不丢字）
  const planned = R.planSubtitleLines(text, ['粉色限定登场。'])
  assert.equal(strip(planned.join('')), strip(text))
  // 去标点后「低延迟稳定传输」=7 字 < 下限 8 → 残片并入前行（合并规则正确行为）
  assert.deepEqual(planned, ['粉色限定登场低延迟稳定传输'])
  // 加字 → 拼接一致性不过 → 回退本地
  const p2 = R.planSubtitleLines(text, [...llm, '多余行'])
  assert.ok(!p2.includes('多余行'))
  assert.equal(strip(p2.join('')), strip(text))
})

test('mapLinesToTiming：句界精确+句内按字数比例；拼接不一致按总时长均分；无 timing 零时长', () => {
  const timing = [
    { text: '第一句，共四字。', start: 0, end: 4 },
    { text: '第二句，共四字。', start: 4, end: 8 },
  ]
  const rows = R.mapLinesToTiming(['第一句，', '共四字。', '第二句，共四字。'], timing)
  assert.equal(rows.length, 3)
  assert.equal(rows[0].start, 0)
  assert.equal(rows[0].end, 2)   // 句内 4/8 字符比例
  assert.equal(rows[1].start, 2)
  assert.equal(rows[1].end, 4)   // 句界精确
  assert.equal(rows[2].start, 4)
  assert.equal(rows[2].end, 8)
  // 拼接不一致 → [0,totalEnd] 按有效字数均分
  const fb = R.mapLinesToTiming(['aaa', 'bbb'], [{ text: '完全不同的文本', start: 0, end: 10 }])
  assert.equal(fb[0].start, 0)
  assert.equal(fb[0].end, 5)
  assert.equal(fb[1].end, 10)
  // 无 timing → 0 时长（end 钳 +0.2 防零长 cue）
  const z = R.mapLinesToTiming(['aa'], [])
  assert.equal(z[0].start, 0)
  assert.equal(z[0].end, 0.2)
})

test('serializeSrtRows/srtTimestamp：与导出链路原内联格式逐字同口径', () => {
  assert.equal(R.srtTimestamp(0), '00:00:00,000')
  assert.equal(R.srtTimestamp(62.236), '00:01:02,236')
  assert.equal(R.serializeSrtRows([{ text: '一', start: 0, end: 1.5 }]),
