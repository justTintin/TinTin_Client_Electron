...
//       app-main（router-view）+ app-footer（44px）
// ═══════════════════════════════════════════════════════════════
...
// 底栏最多展示 3 条下载任务
const visibleDownloads = computed(() => downloadsStore.tasks.slice(0, 3))
const overflowCount = computed(() =>
  Math.max(0, downloadsStore.tasks.length - 3)
)
...
/** F4 热键：回退到系统标题栏（规格 A5 兜底） */
function _onF4(ev: KeyboardEvent): void {
  if (ev.key !== 'F4') return
  if (!isElectronShell.value) return
  // 通过 config IPC 写入 electron-store，然后 relaunch
  ;(async () => {
    try {
      const ok = await (window as any).tintin?.config?.set?.('window.useSystemFrame', true)
      if (ok === false) return
      try { await (window as any).tintin?.app?.relaunch?.() } catch (_) {}
    } catch (_) {}
  })()
}
window.addEventListener('keydown', _onF4)
  ...
  <div class="app-shell" :class="{ 'electron-shell': isElectronShell }">
    ...
    >
      ...
      <div class="title-bar-drag">
        <div class="title-bar-brand">
          ...
          <span class="title-bar-title">螺丝钉-电商智能体矩阵 V{{ appStore.version }}</span>
          <span class="title-bar-f4-hint" title="按 F4 可切换回系统标题栏">F4 可切回系统边框</span>
        </div>
      </div>

      <!-- 右侧：三控件（-webkit-app-region: no-drag，禁止拖拽） -->
      <div class="title-bar-controls" aria-label="窗口控制">
        ...
        <!-- 最大化 / 还原（图标随 winState.maximized 切换） -->
        <button
          class="wc-btn wc-max"
          type="button"
    ...
    <header class="app-header">
      <div class="brand">
        ...
        <div class="brand-text">
          ...
          <span class="brand-version">V{{ appStore.version }}</span>
        </div>
      </div>
      ...
      <div class="header-right">
        ...
        <button class="luo-btn icon size-sm notification-btn" title="通知">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" />
            <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0" />
          </svg>
          <span v-if="notificationCount > 0" class="luo-badge sm status-error notification-dot">{{ notificationCount }}</span>
        </button>
      </div>
    </header>
    ...
    <footer class="app-footer">
      <div class="footer-downloads">
        <template v-if="visibleDownloads.length">
          <div
            v-for="task in visibleDownloads"
            :key="task.taskId"
            class="download-item"
            :title="task.url"
          >
            <span class="download-name">{{ task.savePath }}</span>
            <div class="download-bar">
              <div
                class="download-bar-inner"
                :style="{ width: task.percent + '%' }"
              ></div>
            </div>
            <span class="download-percent">{{ task.percent.toFixed(0) }}%</span>
          </div>
          <span v-if="overflowCount > 0" class="download-overflow">
            +{{ overflowCount }} 个任务进行中
          </span>
        </template>
        <span v-else class="footer-empty">暂无下载任务</span>
      </div>
    </footer>
  </div>
</template>
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
.title-bar {
  flex: 0 0 var(--size-title-bar-height);
...
/* C3：拖拽区使用 -webkit-app-region: drag */
.title-bar-drag {
  flex: 1 1 auto;
  min-width: 0;
  display: flex;
...
.wc-btn:hover { background: var(--surface-container); color: var(--foreground); }
...
/* ─── 底栏 ─── */
.app-footer {
  flex: 0 0 var(--size-page-footer-height);
  height: var(--size-page-footer-height);
  display: flex;
  align-items: center;
  padding: 0 var(--space-4);
  background: var(--surface);
