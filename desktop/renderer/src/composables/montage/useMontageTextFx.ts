...
export function useMontageTextFx(ctx: MontageTextFxContext) {
  ...
  // extractFancyWordsInLine/FromText 纯函数保留于 montageStep4FxBgmLogic.ts，仅供测试
  // 与未来潜在消费）
  ...
  /** 交互标注数据（2026-09-23 用户裁决）：每视频一块——字幕行（时间戳）+ 合并词表，
   *  供「字幕关键词标注」面板渲染彩色命中与右键标注 */
  ...
  // ── 手工标注关键词（2026-09-23 用户裁决）：字幕文本上选中→右键「标注为关键词」，
  //  右键已标注词→「取消标注」；按视频路径（planKey）存储并 localStorage 持久化。
  //  词表优先级=手工 → 产品关联 → LLM 补足（命中判定沿用 matchKeywordHits 词表序）──
  const MANUAL_KW_LS_KEY = 'montage.textfx.manualKeywords'
  ...
  /** 关键词判定 v2（2026-09-23 用户裁决）：词表优先级=手工标注（planKey 维度）→
   *  产品关联词 → LLM 补足；命中词覆盖不足 min(3, 行数) 时自动追加 LLM 提取词
   *  重匹配（保证每条至少 3 词可命中，修「第二条关键词为 0 且无兜底」报障）。
   *  返回 rows（字幕行）+ hits（命中）+ words（合并词表，供交互标注面板）；
   *  原门面 resolveKeywordHits(text, timing, planKey) 保留=只返回 hits。 */
  ...
  async function refreshTextFxTracks(): Promise<void> {
    ...
    if (!textFxEnabled.value) { textFxPreviewTracks.value = []; textFxAnnotate.value = []; return }
