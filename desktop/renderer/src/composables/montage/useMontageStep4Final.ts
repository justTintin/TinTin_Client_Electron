// ═══════════════════════════════════════════════════════════════
// useMontageStep4Final.ts — 智能混剪 Step4 特效包装/BGM/剪映导出编排（铁律 10 拆分，2026-09-19）
...
import { useMontageBgmPlayer } from './useMontageBgmPlayer'

type Step3Api = ReturnType<typeof useMontageStep3Voice>
...
export interface MontageStep4Context {
  ...
  runDubBatch: Step3Api["runDubBatch"]
  ...
  selectedFancyTemplate: Step3Api["selectedFancyTemplate"]
  selectedSubtitlePreset: Step3Api["selectedSubtitlePreset"]
  selectedFontFamily: Step3Api["selectedFontFamily"]
  ...
  clearVoiceProgressListener: Step3Api["clearVoiceProgressListener"]
...
export function useMontageStep4Final(ctx: MontageStep4Context) {
  const {
  ...
  // ── AI 生成 BGM（已迁 montage/useMontageBgmGen.ts，铁律 10 纯搬迁）──
  const bgmGen = useMontageBgmGen({ ensureServerUrl, toAbsolute, voiceDirInput, bgmPath, bgmName })
  const {
  ...
  // ── BGM 试听播放器（已迁 montage/useMontageBgmPlayer.ts，铁律 10 纯搬迁）──
  const bgmPlayer = useMontageBgmPlayer({ bgmPath, bgmVolume })
